...
db.once('open', () => {
  ...
  for (let key in courses) {
    let co = courses[key];
    //TODO: Want to not always add a course, down below?
    wrapped.push(co);
    
    let alreadyInDatabase = [];

    Course.findOne({courseCode: co.courseCode}, {results: 1}).exec((err, result) => {
      if (err) {
        console.log(err);
        return;
      }
      for(let i in result.results) {
        //console.log(result.results[i].date);
        alreadyInDatabase.push(result.results[i].date);
      }

      //console.log(alreadyInDatabase);
      
      co.results.forEach((result) => {
          if(!alreadyInDatabase.includes(result.date)) {
            results.push(result)
          }else{
            console.log("Result already in database " + result.date);
          }
        }
      );
    ...
    //TODO: Want to only push results not already in database
    
  }
  /*
  for(i = 0; i < wrapped.length; i++) {
    let course = wrapped[i];
    console.log(`Course: ${course.courseCode}`)
    console.log(course.results);

    if (course.results.length != 0 ){
        for(j = 0; j < course.results.length; j++) {
            var filterData = results.filter(d => d._id === course.results[j]._id);
